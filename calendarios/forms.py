...
class AddClientForm(forms.Form):  # REQUIRED is True by DEFAULT
    ...
    cantidad_adultos = forms.IntegerField(
        label="Cantidad de adultos (a partir de 13 años)",
        initial=0,
        min_value=1,
        max_value=10,
        widget=forms.NumberInput(attrs={"class": "form-control"}),
    )
    cantidad_menores = forms.IntegerField(
        label="Cantidad de niños (entre 2 a 12 años)",
        initial=0,
        min_value=0,
        max_value=10,
        widget=forms.NumberInput(attrs={"class": "form-control"}),
    )
    cantidad_gratis = forms.IntegerField(
        label="Cantidad de menores de 23 meses",
        initial=0,
        min_value=0,
        max_value=10,
        widget=forms.NumberInput(attrs={"class": "form-control"}),
    ...
    tipo_adelanto = forms.ChoiceField(
        label="Medio de seña",
        choices=[("", "Seleccione un metodo"), (1, "Giro"), (2, "Depósito bancario")],
        widget=forms.Select(attrs={"class": "form-control"}),
    )
    notas = forms.CharField(
        required=False,
        label="Notas",
        widget=forms.Textarea(attrs={"class": "form-control", "size": 1}),
    )
    edit = forms.BooleanField(
        label="Editar", required=False, widget=forms.HiddenInput()
    )
    confirm = forms.BooleanField(
        label="Confirm", required=False, widget=forms.HiddenInput()
    )
    ...
    def clean(self):
        ...
        if fecha_fin and fecha_inicio:
            if not ((fecha_fin - fecha_inicio) >= timedelta(days=0)):
                raise forms.ValidationError(
                    "ERROR: La fecha final es antes de la fecha inicial."
                )
            if casa == "":
            ...
            if not edit:
                ...
                if (
                    ...
                    ).filter(casa=int(casa))
                    or Reservas.objects.filter(
                        fecha_fin__range=(fecha_inicio, fecha_fin)
                    ).filter(casa=int(casa))
                    or Reservas.objects.filter(fecha_inicio__gte=fecha_inicio)
                    .filter(fecha_fin__lte=fecha_fin)
                    .filter(casa=int(casa))
                ):
                    raise forms.ValidationError(
                        "ERROR: El rango de fechas seleccionado no está disponible para esta casa."
                    )
        ...
        if (
            cantidad_adultos + cantidad_menores <= 0
            or cantidad_adultos + cantidad_menores > 10
        ):
            raise forms.ValidationError(
                "ERROR: La cantidad de personas total es inválida. El máximo es 10."
            )
...
class ChangePaymentForm(forms.Form):
    ...
    cantidad_deposito = forms.IntegerField(
        label="Cantidad",
        min_value=0,
...
class SearchReservationForm(forms.Form):
    ...
    def clean(self):
        ...
        if not query and not fecha:
            raise forms.ValidationError(
                "ERROR: Es necesario completar por lo menos uno de los campos."
            )
