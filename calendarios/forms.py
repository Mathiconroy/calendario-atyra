...
class AddClientForm(forms.Form): # If let blank, they are REQUIRED by DEFAULT
    ...
    edit = forms.BooleanField(label='Editar', required=False) # TODO: ADD THE HIDDENINPUT WIDGET!!!
    ...
    def clean(self):
        ...
        if (fecha_fin and fecha_inicio and cantidad_dias) or fecha_fin and fecha_inicio and cantidad_dias == 0:
            ...
            if not edit:
                ...
                r = Reservas.objects.filter(fecha_inicio__gte=fecha_inicio).filter(fecha_fin__lte=fecha_fin).filter(casa=int(casa)).exclude(id=id)
                # TODO: Fix this, for some reason it returns expected results if fecha_fin + 1
                # LOL THIS ALGORITHM IS WRONG FUCK
                print(fecha_inicio)
                print(fecha_fin)
                print(r)
                if r:
