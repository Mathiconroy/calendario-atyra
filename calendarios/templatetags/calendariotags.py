        ...
        if reserva:
            if reserva.estado == 0:
                ...
                if request.user.is_authenticated:
                    ...
                    text = "Reservado"
            if request.user.is_authenticated:
                table_row = table_row + format_html(
                    "<td class='calendario-row-data' bgcolor={}>{} <strong><a href='/view_client_form/{}'>{}</a></strong> ({} personas)</td>",
                    color,
                    text,
                    reserva.id,
                    reserva.nombre,
                    reserva.cantidad_adultos
                    + reserva.cantidad_menores
                    + reserva.cantidad_gratis,
                )
            else:
                table_row = table_row + format_html(
                    "<td class='calendario-row-data' bgcolor={}>{}</td>",
                    color,
                    text,
                )
        else:
...
def render_confirm(
    dictionary, key
):  # The dictionary contains submitted ReservaForm values
    correct_names_dict = {
    ...
    if key == "casa":
        ...
        return (
            f"{correct_names_dict[key]}: {tipos_adelanto.get(int(dictionary.get(key)))}"
        )
