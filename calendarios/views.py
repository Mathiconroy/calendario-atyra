...
def index(request):
    ...
    reservas = (
        Reservas.objects.filter(
            fecha_inicio__lte=date.today() + timedelta(days=days_to_check_count)
        )
        .exclude(fecha_fin__lt=date.today())
    ...
    return render(
        request,
        "calendarios/main.html",
        {"date_list": date_list, "dias_ocupados": dias_ocupados_dict},
    )
        ...
        if form.is_valid():
            ...
            if form_results["confirm"] is False:
                ...
                messages.add_message(
                    ...
                    extra_tags="alert alert-warning text-center",
                )
                precio = calculate_price(
                    int(form_results["cantidad_adultos"]),
                    int(form_results["cantidad_menores"]),
                )
                remove_not_used_fields(form_results)
                return render(
                    request,
                    "calendarios/form.html",
                    {
                        "form": form,
                        ...
                        "reserva_length": (
                            form_results["fecha_fin"] - form_results["fecha_inicio"]
                        ).days,
                    },
                )
            else:
                ...
                if request.user.is_authenticated:
                    ...
                    r.estado = 0
                    form_results["url"] = request.build_absolute_uri(
                        "/view_client_form/" + str(r.id)
                    )
                    messages.add_message(
                        request,
                        messages.SUCCESS,
...
def search_reservation(request):
    ...
    return render(
        request, "search_reservation.html", {"form": form, "reservas": reservas}
    )
...
def change_payment(request, id):
    ...
    return render(
        request,
        "calendarios/change_payment_form.html",
        {
            "form": form,
            "reserva": r,
...
def view_reservation_requests(request):
    ...
    print(r)
    return render(
        request, "calendarios/view_reservation_requests.html", {"reservas": r}
    )
